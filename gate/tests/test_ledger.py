...
# --- F15-T5 as reversed by Q14: a signer earns the proof line (D-9, D-21 v3.17) ----------------


def test_a_signer_earns_the_proof_line(tmp_path: Path) -> None:
    """AC5 as rescinded: an identity holding a counting signature on the target's root earns the
    proof line when their proof merges, like anyone but the curator; the bar drafted as R5 was
    withdrawn before release (Q14), so the ledger has no ``signer_bar``."""
    ...
    assert not hasattr(ledger, "signer_bar")
    ...
    # The curator's bar is untouched (D-21).
    earned, skipped, _doc = cli._merge_entries(
        root, classification, identity="curator", commit="a" * 40, date="2026-09-16T00:00:00Z",
