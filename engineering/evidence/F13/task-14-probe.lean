...
theorem demo : ∀ (n : ℕ) (h : 2 ≤ n) (x : ℝ), 0 < x → ∀ k : ℕ, x ^ k > 0 := by
  sorry

theorem witness : ∃ (n : ℕ) (_ : 2 ≤ n) (x : ℝ) (k : ℕ), 0 < x := ⟨2, le_refl 2, 1, 0, one_pos⟩
...
open Lean Meta in
run_meta do
  let s ← getConstInfo `demo
  let expected ← OpnGate.expectedWitnessType s.type
  let pp (e : Expr) : MetaM String :=
    withOptions (fun o => ((o.setBool `pp.coercions.types true).setBool `pp.numericTypes true).setBool `pp.funBinderTypes true) do
      return toString (← ppExpr e)
  let w ← getConstInfo `witness
  let ok ← withReducible (isDefEq expected w.type) <||> isDefEq expected w.type
  logInfo m!"OPN-WITNESS {Json.compress (Json.mkObj [("expected", Json.str (← pp expected)), ("given", Json.str (← pp w.type)), ("matches", Json.bool ok)])}"
